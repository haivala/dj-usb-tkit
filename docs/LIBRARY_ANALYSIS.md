...
The beat-grid anchor `first_beat_ms` is produced in two stages when audio
decodes successfully: the selected analysis engine (`stratum-dsp` or the
legacy `essentia.js`) reports a coarse beat-grid estimate, which is then
refined by `beat_refine::refine_first_beat_ms` -- a transient-snap pass that
re-examines the raw decoded samples around (and, if that estimate turns out
to sit in genuine leading silence, well beyond) the engine's reported time
and snaps to the track's real attack transient when one is found with enough
confidence. If audio decode fails, `anlz::estimate_first_beat_ms` falls back
to a lower-resolution, waveform-cache-based phase estimate instead. The
result is **user-editable** from the track-detail ("Cues") modal alongside
cue points. Once edited, `first_beat_ms_source` flips to `'user'` and
re-analysis keeps the user's value instead of re-estimating. See
`docs/APP_DATA_MODEL.md` (TrackCue) and `docs/USB_EXPORT.md`.
