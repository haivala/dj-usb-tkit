...
- **Fix:** when an existing PDB track is matched by alias-normalized path identity, rewrite its indexed media path and filename slot from the manifest exactly. This prevents exports from leaving PDB/eDB pointing at the old `Track - .mp3` variant while the on-disk file is `Track -.mp3`, which showed up as an unindexed audio file in USB diagnostics.
