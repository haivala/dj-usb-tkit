...
fn user_like_flow_imports_sources_analyzes_and_adds_from_library_usb_and_history() {
    ...
    // pruning (protects against unmounted drives being mistaken for deleted
    // media), so a genuine per-file removal is the only way to exercise
    // pruning here — see scan_missing_source_root_reports_not_found_without_pruning_tracks
    // in scan_incremental_functional.rs for the other half of that contract.
    fs::remove_file(source_b.join("Artist - C132.wav")).expect("remove source-b track file");
