...
fn insert_track_usb_link(conn: &rusqlite::Connection, track_id: &str, device_id: &str, usb_file_path: &str) {
    conn.execute(
...
fn merge_orphaned_usb_placeholder_tracks_reassigns_links_and_deletes_placeholder() {
    ...
    insert_track(&conn, "placeholder-1", "/mnt/usb1/Contents/a.mp3", "fp1", Some(200_500), Some(1_000));
    ...
    let placeholder_gone: i64 = conn
        .query_row("SELECT COUNT(1) FROM tracks WHERE id = 'placeholder-1'", [], |row| row.get(0))
        .expect("count");
    ...
    let playlist_track_id: String = conn
        .query_row("SELECT track_id FROM playlist_tracks WHERE playlist_id = 'pl-1'", [], |row| row.get(0))
...
fn merge_orphaned_usb_placeholder_tracks_skips_diverging_duration_or_size() {
    ...
    // Diverging duration.
    insert_track(&conn, "local-a", "/music/a.mp3", "fp-a", Some(200_000), Some(1_000));
    insert_track(&conn, "placeholder-a", "/mnt/usb1/Contents/a.mp3", "fp-a", Some(210_000), Some(1_000));

    // Matching duration, diverging size (Clean/Explicit collision case).
    insert_track(&conn, "local-b", "/music/b.mp3", "fp-b", Some(200_000), Some(1_000));
    insert_track(&conn, "placeholder-b", "/mnt/usb1/Contents/b.mp3", "fp-b", Some(200_000), Some(999));

    // Two local candidates tie -- ambiguous, must not auto-merge.
    insert_track(&conn, "local-c1", "/music/c1.mp3", "fp-c", Some(200_000), Some(1_000));
    insert_track(&conn, "local-c2", "/music/c2.mp3", "fp-c", Some(200_000), Some(1_000));
    insert_track(&conn, "placeholder-c", "/mnt/usb1/Contents/c.mp3", "fp-c", Some(200_000), Some(1_000));
    drop(conn);
...
fn merge_orphaned_usb_placeholder_tracks_skips_placeholder_with_missing_file_size() {
    ...
    insert_usb_device(&conn, "dev-1", "/mnt/usb1");
    insert_track(&conn, "local-1", "/music/a.mp3", "fp1", Some(200_000), Some(1_000));
    insert_track(&conn, "placeholder-1", "/mnt/usb1/Contents/a.mp3", "fp1", Some(200_000), None);
    drop(conn);
...
fn merge_orphaned_usb_placeholder_tracks_is_idempotent() {
    ...
    let first = svc.merge_orphaned_usb_placeholder_tracks().expect("first merge");
    assert_eq!(first.merged, 1);
    let second = svc.merge_orphaned_usb_placeholder_tracks().expect("second merge");
...
fn resolve_playback_source_excludes_usb_placeholder_rows_even_when_no_local_copy_exists() {
    ...
    let conn = svc.db.connect().expect("connect");
    insert_usb_device(&conn, "dev-1", "/mnt/usb1");
    insert_track(&conn, "placeholder-1", "/mnt/usb1/Contents/a.mp3", "", Some(200_000), Some(1_000));
    conn.execute(
...
fn resolve_playback_source_treats_path_as_local_once_it_is_a_configured_source_root() {
    ...
    insert_track(&conn, "track-1", "/mnt/data/a.mp3", "", Some(200_000), Some(1_000));
...
fn resolve_playback_source_excludes_candidate_with_mismatched_file_size() {
    ...
    insert_track(&conn, "wrong-size", "/music/wrong.mp3", &fp, Some(200_000), Some(999));
    insert_track(&conn, "right-size", "/music/right.mp3", &fp, Some(200_000), Some(1_000));
    conn.execute(
...
fn resolve_playback_source_ignores_file_size_veto_when_either_side_unknown() {
    ...
    insert_track(&conn, "only-candidate", "/music/only.mp3", &fp, Some(200_000), None);
    conn.execute(
...
fn resolve_playback_source_falls_through_when_track_id_is_usb_rooted() {
    ...
    let conn = svc.db.connect().expect("connect");
    insert_usb_device(&conn, "dev-1", "/mnt/usb1");
    let fp = backend::service::build_track_match_fingerprint("Stale Playlist Entry", "Artist", None);
    insert_track(&conn, "local-1", "/music/a.mp3", &fp, None, None);
    insert_track(&conn, "placeholder-1", "/mnt/usb1/Contents/a.mp3", &fp, None, None);
    conn.execute(
    ...
    let resolved = svc
        .resolve_playback_source(resolve_req("Stale Playlist Entry", "Artist", Some("placeholder-1")))
        .expect("resolve");
    assert_ne!(
...
fn resolve_playback_source_fast_path_handles_missing_track_id() {
    ...
    let resolved = svc
        .resolve_playback_source(resolve_req("Findable Track", "Artist", Some("does-not-exist")))
        .expect("resolve");
    assert_eq!(resolved.matched_by, "hash");
    assert_eq!(resolved.resolved_path.as_deref(), Some("/music/a.mp3"));
}
...
fn prune_usb_device_soft_deletes_without_breaking_existing_links() {
    ...
    let pruned = svc
        .prune_usb_device(backend::models::PruneUsbDeviceRequest { id: "dev-1".to_string() })
        .expect("prune");
    assert!(pruned.pruned);

    let conn = svc.db.connect().expect("connect");
    let deleted_at: Option<String> = conn
        .query_row("SELECT deleted_at FROM usb_devices WHERE id = 'dev-1'", [], |row| row.get(0))
        .expect("deleted_at");
    assert!(deleted_at.is_some());

    let listed = svc.list_usb_devices().expect("list");
    assert!(listed.items.is_empty(), "pruned device must not appear in list_usb_devices");

    let link_count: i64 = conn
        .query_row("SELECT COUNT(1) FROM track_usb_links WHERE usb_device_id = 'dev-1'", [], |row| row.get(0))
        .expect("link count");
    assert_eq!(link_count, 1, "pruning must not cascade-delete track_usb_links");
    let export_count: i64 = conn
        .query_row("SELECT COUNT(1) FROM usb_device_exports WHERE usb_device_id = 'dev-1'", [], |row| row.get(0))
        .expect("export count");
    assert_eq!(export_count, 1, "pruning must not cascade-delete usb_device_exports");
    ...
    let root_still_present: i64 = conn
        .query_row("SELECT COUNT(1) FROM usb_devices WHERE root_path = '/mnt/usb1'", [], |row| row.get(0))
    ...
    // it's cleared).
    conn.execute("UPDATE usb_devices SET deleted_at = NULL WHERE id = 'dev-1'", [])
